...
$checks = array(
    array(
        ...
        'query' => 'SELECT actions.date, actions.amount, actions.note, actions.po, l1.name AS t_loc_name, accounts.name, l2.name AS a_loc_name, contacts.name AS c_name, contacts.company, actions.actionid FROM actions LEFT JOIN location AS l1 USING (locationid) LEFT JOIN contacts USING (contactid) LEFT JOIN accounts USING (accountid) LEFT JOIN location AS l2 ON (accounts.locationid = l2.locationid) LEFT JOIN (SELECT accountid, MAX(timestamp) AS timestamp, GROUP_CONCAT(DISTINCT old_value) AS old_values, GROUP_CONCAT(DISTINCT new_value) AS new_values FROM modlog_accounts WHERE field = "locationid" GROUP BY accountid ) AS mla ON (mla.accountid = accounts.accountid AND old_values LIKE CONCAT("%",actions.locationid,"%") AND new_values LIKE CONCAT("%",accounts.locationid,"%") AND mla.timestamp >= actions.udate) WHERE actions.in_kind = 0 AND actions.locationid <> accounts.locationid AND mla.timestamp IS NULL',
        'sql_params' => array(),
        'rows' => array(),
    ),
);
