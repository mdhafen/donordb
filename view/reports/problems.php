...
<?php
      if ( !empty($data['op']) ) {
        $count = 0;
?>
<?php   foreach ( $data['checks'] as $report ) { ?>
    <div id="reports_table_container_<?= ++$count ?>">
      <h3><?= $report['legend'] ?> </h3>
      <ul class="paginationTop_<?= $count ?>"></ul>
      <table class="uk-table" id="report_table_<?= $count ?>">
<?php
   if ( !empty($report['columns']) ) {
     print "<thead>\n<tr>\n";
     foreach ( $report['columns'] as $row ) {
?>
...
<?php
   if ( !empty($report['rows']) ) {
     ...
     foreach ( $report['rows'] as $row ) {
       if ( !empty($row[0]['new_group']) && $row_count != 1 ) {
...
?>
    ...
    <ul class="paginationBottom_<?= $count ?>"></ul>
  </div>
<script>
var list_options = {
  valueNames: [ <?php 
...
<?php if ( $data['paged'] !== false ) { ?>
  ...
  plugins: [
    ListPagination({
      name: "paginationTop_<?= $count ?>",
      paginationClass: "paginationTop_<?= $count ?>",
      innerWindow: 2,
      outerWindow: 1
    }),
    ListPagination({
      name: "paginationBottom_<?= $count ?>",
      paginationClass: "paginationBottom_<?= $count ?>",
      innerWindow: 2,
      outerWindow: 1
    })
  ]
...
var list_obj = new List('reports_table_container_<?= $count ?>', list_options);
</script>
<?php   } ?>
<?php
} else {
